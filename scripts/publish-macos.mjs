...
assert.equal(
  JSON.parse(gh('api', `repos/${repo}/releases/latest`)).id,
  release.id,
...
feed.notes = notes;
feed.releaseVersion = releaseVersion;
...
const oldChecksums = readFileSync('existing-release/SHA256SUMS.txt', 'utf8')
  .trim()
  .split(/\r?\n/)
