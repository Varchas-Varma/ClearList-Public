...
import { branches, canPlaceMany } from '../tree';
import { placeSelection, selectionFor } from '../state/selection';
...
const collision: CollisionDetection = (args) => {
  ...
  const ids = source?.kind === 'task' ? selectionFor(source.id) : [];
  const excluded = branches(data.tasks, ids);
  const containers = args.droppableContainers.filter((c) => {
    ...
    return target.kind === 'gap' && canPlaceMany(data.tasks, ids, target);
  });
  ...
  return (
    <DndContext
      ...
      onDragStart={({ active }) => {
        ...
        if (item.kind === 'task' && !appStore.getState().selectedTaskIds.includes(item.id))
          appStore.getState().selectTask(item.id);
        setActive(item);
        useTreeUi.setState({
      ...
      onDragEnd={({ active, over }) => {
        ...
        if (target.kind === 'list-drop') {
          void placeSelection(selectionFor(task.id), {
            listId: target.id,
            parentId: null,
            beforeId: null,
            completed: null,
      ...
      <DragOverlay dropAnimation={null}>
        {active && (
          <div className="drag-overlay">
            {active.kind === 'task' && selectionFor(active.id).length > 1
              ? `${selectionFor(active.id).length} selected tasks`
              : active.label}
          </div>
        )}
      </DragOverlay>
